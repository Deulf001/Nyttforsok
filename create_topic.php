...
{
	echo 'Du måste vara inloggad <a href="signin.php">Logga in</a> för att skapa en topic.';
}
else
{
	if($_SERVER['REQUEST_METHOD'] != 'POST')
	{
		...
		$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
			if($stmt->rowCount() == 0) 			{
				if($_SESSION['level'] == 1)
				{
					echo 'Du har inga kategorier �nnu';
				}
				else
				{
					echo 'Innan du skapar en topic måste admin ha skapat en kategori';
				}
			}
			else
			{
				echo '<form method="post" action="">
					�mne: <input type="text" name="subject" />
					<br>Kategori:';
	...
	else
	{
			...
			$row = $stmt->fetchAll(PDO::FETCH_ASSOC);			
				...
				{
					echo 'Nu blev det lite fel med din topic, f�rs�k igen';
				}
